  ...
  #access by index (CONSTS[0]), name (CONSTS.WIDTH) or via unpacking (a,b,.. = CONSTS) 
    ...
    while running:
        ...
        if not train: #deploy model
            pass #EDIT:
            # X = [b0.x, b0.y, b0.vx, b0.vy, dir, Ball.RADIUS, Paddle.L, Paddle.STEP, CONSTS.WIDTH, CONSTS.HEIGHT,
            #      CONSTS.BORDER, CONSTS.VELOCITY, CONSTS.FPS]
            # print(X)
            # exit
            # break
            #train_file.writerow([b0.x, b0.y, b0.vx, b0.vy, dir, p0.y, Ball.RADIUS, Paddle.L, Paddle.STEP, CONSTS.WIDTH, CONSTS.HEIGHT, CONSTS.BORDER, CONSTS.VELOCITY, CONSTS.FPS])
            X = np.array([b0.x, b0.y, b0.vx, b0.vy, dir]).reshape(1,-1)
            y = model.predict(X)
            if p0.y>y:
               dir = -1
            elif p0.y<y:
